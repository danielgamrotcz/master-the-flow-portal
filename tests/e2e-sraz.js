...
(async () => {
  ...
  check('Tři nově pojmenované bloky čekají na téma a popis', /14:25–14:45[\s\S]*Martin Pavlíček[\s\S]*Téma připravujeme[\s\S]*Popis bloku připravujeme/.test(programText) && /15:05–15:25[\s\S]*Aneta Martinek[\s\S]*Téma připravujeme[\s\S]*Popis bloku připravujeme/.test(programText) && /15:25–15:45[\s\S]*Vojtěch Veselý[\s\S]*Téma připravujeme[\s\S]*Popis bloku připravujeme/.test(programText) && (programText.match(/Téma připravujeme/g) || []).length === 3 && (programText.match(/Popis bloku připravujeme/g) || []).length === 3);
