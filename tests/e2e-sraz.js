...
(async () => {
  ...
  check('Úvod programu shrnuje všech pět hostů', /Postupně vystoupí Alex Trejtnar, Martin Pavlíček, Tomáš „Vilík“ Pospíchal, Aneta Martinek a Vojtěch Veselý/.test(programText) && /Témata Martina, Anety a Vojtěcha doplním, jakmile je doladíme/.test(programText));
  ...
  check('Tři nově pojmenované bloky čekají jen na téma a mají medailonek', /14:25–14:45[\s\S]*Martin Pavlíček[\s\S]*Téma připravujeme[\s\S]*Director v poradenské divizi Deloitte[\s\S]*Přes třináct let/.test(programText) && /15:05–15:25[\s\S]*Aneta Martinek[\s\S]*Téma připravujeme[\s\S]*#HolkyzMarketingu[\s\S]*efektivita a produktivita/.test(programText) && /15:25–15:45[\s\S]*Vojtěch Veselý[\s\S]*Téma připravujeme[\s\S]*R&D startupu[\s\S]*Gemini i Claude[\s\S]*malých iteracích/.test(programText) && (programText.match(/Téma připravujeme/g) || []).length === 3 && !/Popis bloku připravujeme/.test(programText));
