...
(async () => {
  ...
  check('Úvod programu shrnuje všech šest hostů a pravdivě vymezuje otevřené téma', /Postupně vystoupí Alex Trejtnar, Martin Pavlíček, Tomáš „Vilík“ Pospíchal, Aneta Martinek, Vojtěch Veselý a Ondřej Tyl/.test(programText) && /Vojtěchovo téma doplním, jakmile je doladíme/.test(programText));
  ...
  check('Anetin blok používá potvrzené téma a participativní formát', /15:05–15:25[\s\S]*Aneta Martinek[\s\S]*AI brain fry a jak si nastavit AI hygienu[\s\S]*#HolkyzMarketingu[\s\S]*#HolkyzByznysu[\s\S]*úvodní myšlenky[\s\S]*prostor tipům ostatních/.test(programText));
  check('Jako připravované zůstává jen Vojtěchovo téma', /15:25–15:45[\s\S]*Vojtěch Veselý[\s\S]*Téma připravujeme[\s\S]*Gemini i Claude[\s\S]*grantů, vývoje, kvality a servisu/.test(programText) && (programText.match(/Téma připravujeme/g) || []).length === 1);
