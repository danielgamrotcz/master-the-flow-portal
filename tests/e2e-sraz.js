...
(async () => {
  ...
  check('Úvod programu shrnuje všech pět hostů a pravdivě vymezuje otevřené detaily', /Postupně vystoupí Alex Trejtnar, Martin Pavlíček, Tomáš „Vilík“ Pospíchal, Aneta Martinek a Vojtěch Veselý/.test(programText) && /Martinovo téma a finální podobu Vojtěchovy ukázky doplním, jakmile je doladíme/.test(programText));
  ...
  check('Martin čeká na téma, Aneta má potvrzený blok a Vojtěch pravdivý pracovní úhel', /14:25–14:45[\s\S]*Martin Pavlíček[\s\S]*Téma připravujeme[\s\S]*Director v poradenské divizi Deloitte[\s\S]*Přes třináct let/.test(programText) && /15:05–15:25[\s\S]*Aneta Martinek[\s\S]*Efektivita a produktivita: úvod a společný brainstorming[\s\S]*#HolkyzMarketingu[\s\S]*reálného provozu/.test(programText) && /15:25–15:45[\s\S]*Vojtěch Veselý[\s\S]*AI v malém R&D týmu: co funguje, co ne a proč[\s\S]*Gemini i Claude[\s\S]*kontrola na člověku/.test(programText) && (programText.match(/Téma připravujeme/g) || []).length === 1 && !/Popis bloku připravujeme/.test(programText));
