...
(async () => {
  ...
  check('Úvod programu shrnuje všech pět hostů a pravdivě vymezuje otevřená témata', /Postupně vystoupí Alex Trejtnar, Martin Pavlíček, Tomáš „Vilík“ Pospíchal, Aneta Martinek a Vojtěch Veselý/.test(programText) && /Témata Martina, Anety a Vojtěcha doplním, jakmile je doladíme/.test(programText));
  ...
  check('Martin, Aneta a Vojtěch čekají na téma a mají jen profilový medailonek', /14:25–14:45[\s\S]*Martin Pavlíček[\s\S]*Téma připravujeme[\s\S]*Director v poradenské divizi Deloitte[\s\S]*Přes třináct let/.test(programText) && /15:05–15:25[\s\S]*Aneta Martinek[\s\S]*Téma připravujeme[\s\S]*#HolkyzMarketingu[\s\S]*#HolkyzByznysu/.test(programText) && /15:25–15:45[\s\S]*Vojtěch Veselý[\s\S]*Téma připravujeme[\s\S]*Gemini i Claude[\s\S]*grantů, vývoje, kvality a servisu/.test(programText) && (programText.match(/Téma připravujeme/g) || []).length === 3 && !/Popis bloku připravujeme|Efektivita a produktivita|společný brainstorming|AI v malém R&D týmu|kontrola na člověku/.test(programText));
