...
(async () => {
  ...
  const syncWithoutD1 = await onRequestPost({
    ...
    request: new Request('http://localhost/api/meetup-attendees', {
      ...
      body: JSON.stringify({ attendees: [{ consent: true, name: 'Účastník pro synchronizaci', bio: 'Bez D1.', attendance: 'official' }], registeredCount: 1 }),
    }),
  });
  ...
  check('Veřejné API vrátí bezpečný počet všech registrací', publicPayload.registeredCount === 7, String(publicPayload.registeredCount));
